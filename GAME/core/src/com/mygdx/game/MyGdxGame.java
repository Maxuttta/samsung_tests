...
public class MyGdxGame extends ApplicationAdapter {
    static final float W=1280,H=720,GRAVITY=-1450;
    SpriteBatch batch; BitmapFont font; Texture pixel, heroTex, enemyTex, diamondTex, crateTex, flagTex, bgTex, cloudsTex, reflectionsTex;
    TextureRegion hero, enemy, diamond, crate, flag, bg, clouds, reflections;
    Music music; Sound destroySound, attackSound; boolean musicEnabled, soundEnabled;
    TextureAtlas uiAtlas; TextureRegion uiButton, uiPanel, uiLeft, uiRight, uiAttack;
    TextureRegion[][] heroFrames, enemyFrames, diamondFrames, crateFrames, flagFrames;
    TiledMap tiledMap; OrthogonalTiledMapRenderer mapRenderer; int mapHeight, mapWidth;
    ExtendViewport viewport; OrthographicCamera camera; Preferences prefs;
    enum State { MENU, SETTINGS, PLAYING, PAUSED, ENDED } State state=State.MENU;
    int selected, unlocked, level, score, lives; float px,py,vx,vy,time,attack,hurt,deathTime; boolean ground, right=true, playerDead; String endText="";
    Rectangle player=new Rectangle(), finish=new Rectangle(); Array<Rectangle> floors=new Array<Rectangle>(), ladders=new Array<Rectangle>(); Array<Item> gems=new Array<Item>(), boxes=new Array<Item>(); Array<Enemy> foes=new Array<Enemy>();
    static class Item { Rectangle r; boolean gone; float breaking=-1; Item(float x,float y,float w,float h){r=new Rectangle(x,y,w,h);} }
    static class Enemy { Rectangle r, hit; float lo,hi,dir=1,vy; boolean grounded, defeated; Enemy(float x,float y,float lo,float hi){r=new Rectangle(x,y,34,30);hit=new Rectangle();this.lo=lo;this.hi=hi;syncHit();}void syncHit(){hit.set(r.x+5,r.y+2,24,25);} }

    @Override public void create(){
        batch=new SpriteBatch(); font=new BitmapFont(); font.getData().setScale(1.25f); camera=new OrthographicCamera(); viewport=new ExtendViewport(W,H,camera);
        ...
        prefs=Gdx.app.getPreferences("pirate-treasure");unlocked=prefs.getInteger("unlocked",0);musicEnabled=prefs.getBoolean("music",true);soundEnabled=prefs.getBoolean("sound",true);music=Gdx.audio.newMusic(Gdx.files.internal("assets/sounds/background_music.mp3"));destroySound=Gdx.audio.newSound(Gdx.files.internal("assets/sounds/destroy.mp3"));attackSound=Gdx.audio.newSound(Gdx.files.internal("assets/sounds/shoot.mp3"));music.setLooping(true);if(musicEnabled)music.play();
    }
    ...
    boolean down(int a,int b){return Gdx.input.isKeyPressed(a)||Gdx.input.isKeyPressed(b);} void horizontal(){for(Rectangle f:floors)if(player.overlaps(f)){px=vx>0?f.x-player.width:f.x+f.width;vx=0;player.setPosition(px,py);}}void vertical(){for(Rectangle f:floors)if(player.overlaps(f)){if(vy<=0){py=f.y+f.height;ground=true;}else py=f.y-player.height;vy=0;player.setPosition(px,py);}}
    ...
    void start(int n){
        ...
        String path="assets_for_pirate_game/maps/level"+(n+1)+".tmx"; tiledMap=new TmxMapLoader().load(path);mapRenderer=new OrthogonalTiledMapRenderer(tiledMap);
    ...
    void add(float x,float y,float w,float h){floors.add(new Rectangle(x,y,w,h));}void startDeath(String text){playerDead=true;deathTime=0;endText=text;vx=vy=0;}void finish(boolean won,String text){endText=text;if(won){unlocked=Math.max(unlocked,Math.min(2,level+1));prefs.putInteger("unlocked",unlocked).flush();}state=State.ENDED;}
    ...
    void menu(){center("PIRATE TREASURE",635,2.2f,new Color(.13f,.22f,.48f,1));panel(355,315,570,230);center("SELECT AN EXPEDITION",505,1.15f,Color.WHITE);String[] ns={"1  FIRST FLIGHT","2  REVENGE OF PIT","3  THE EMPIRE STRIKES"};for(int i=0;i<3;i++)button(405,445-i*48,470,38,ns[i]+(i>unlocked?"  (-)":""),i<=unlocked?new Color(.88f,.53f,.30f,1):new Color(.35f,.35f,.40f,1));button(405,250,470,48,"START THIS GAME",new Color(.20f,.58f,.42f,1));button(405,185,220,42,"EXIT",new Color(.78f,.38f,.25f,1));button(655,185,220,42,"SETTINGS",new Color(.78f,.38f,.25f,1));if(Gdx.input.justTouched()){if(touch(405,445,470,38))selected=0;else if(touch(405,397,470,38)&&unlocked>=1)selected=1;else if(touch(405,349,470,38)&&unlocked>=2)selected=2;else if(touch(405,250,470,48))start(selected);else if(touch(655,185,220,42))state=State.SETTINGS;else if(touch(405,185,220,42))Gdx.app.exit();}}
    void settings(){center("SETTINGS",620,2f,new Color(.13f,.22f,.48f,1));panel(425,235,430,270);center("Progress and audio",465,1f,Color.WHITE);button(480,395,320,40,"RESET LEVELS",new Color(.78f,.38f,.25f,1));button(480,345,320,40,"MUSIC: "+(musicEnabled?"ON":"OFF"),new Color(.20f,.58f,.42f,1));button(480,295,320,40,"SFX: "+(soundEnabled?"ON":"OFF"),new Color(.20f,.58f,.42f,1));button(480,245,320,40,"HOME",new Color(.78f,.38f,.25f,1));if(Gdx.input.justTouched()){if(touch(480,395,320,40)){unlocked=0;prefs.putInteger("unlocked",0).flush();}else if(touch(480,345,320,40)){musicEnabled=!musicEnabled;prefs.putBoolean("music",musicEnabled).flush();if(musicEnabled)music.play();else music.pause();}else if(touch(480,295,320,40)){soundEnabled=!soundEnabled;prefs.putBoolean("sound",soundEnabled).flush();}else if(touch(480,245,320,40))state=State.MENU;}}
    void drawMapWorld(){
