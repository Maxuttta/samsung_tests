...
public class MyGdxGame extends ApplicationAdapter {
    ...
    private Rectangle player=new Rectangle(), finish=new Rectangle(); private Array<Rectangle> floors=new Array<Rectangle>(), ladders=new Array<Rectangle>(); private Array<Collectible> gems=new Array<Collectible>(), boxes=new Array<Collectible>(); private Array<EnemyEntity> foes=new Array<EnemyEntity>();
    ...
    @Override public void render(){float dt=Math.min(Gdx.graphics.getDeltaTime(),.033f);time+=dt;session.advance(dt);if(session.mode()==GameMode.PLAYING)update(dt);Gdx.gl.glClearColor(.10f,.22f,.34f,1);Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);viewport.apply();if(session.mode()==GameMode.MENU||session.mode()==GameMode.SETTINGS){camera.zoom=1;camera.position.set(W/2,H/2,0);camera.update();batch.setProjectionMatrix(camera.combined);batch.begin();background();if(session.mode()==GameMode.MENU)menu();else settings();batch.end();return;}drawMapWorld();camera.zoom=1;camera.position.set(W/2,H/2,0);camera.update();batch.setProjectionMatrix(camera.combined);batch.begin();hud();if(session.mode()==GameMode.PAUSED)pauseMenu();if(session.mode()==GameMode.ENDED)ended();batch.end();}
    void update(float dt){
        ...
        for(Collectible i:gems)if(!i.isCollected()&&player.overlaps(i.bounds())){i.collect();score+=50;}
        for(Collectible i:boxes){if(i.breakTime()>=0){i.advanceBreaking(dt);}else if(!i.isCollected()&&player.overlaps(i.bounds())){i.startBreaking();score+=30;if(soundEnabled)destroySound.play(.45f);}}
        Rectangle strike=new Rectangle(right?px+player.width:px-30,py+7,30,24);
        for(EnemyEntity e:foes){if(e.isDefeated())continue;moveEnemy(e,dt);if(attack>0&&strike.overlaps(e.hitBounds())){e.defeat();score+=10;if(soundEnabled)destroySound.play(.35f);}else if(player.overlaps(e.hitBounds())&&hurt<=0){if(--lives<=0){startDeath("Don't touch enemies.\nThey don't like it.");return;}hurt=.9f;vx=px<e.bounds().x?-360:360;}}
        if(player.overlaps(finish)){finish(true,"Our congratulations!");return;}if(py<-80){startDeath("Don't fall into the pit!");return;}if(Gdx.input.isKeyJustPressed(Input.Keys.ESCAPE)||Gdx.input.isKeyJustPressed(Input.Keys.P))session.mode(GameMode.PAUSED);
    }
    ...
    void moveEnemy(EnemyEntity e,float dt){
        e.grounded(false);e.velocityY(e.velocityY()+GRAVITY*dt);e.bounds().y+=e.velocityY()*dt;for(Rectangle f:floors)if(e.bounds().overlaps(f)&&e.velocityY()<=0){e.bounds().y=f.y+f.height;e.velocityY(0);e.grounded(true);}
        if(e.patrolEnd()>e.patrolStart()){float next=e.bounds().x+e.direction()*80*dt;if(next<=e.patrolStart()||next>=e.patrolEnd())e.reverse();else if(e.grounded()&&hasFloorAt(e,next+(e.direction()>0?e.bounds().width:-2)))e.bounds().x=next;else if(e.grounded())e.reverse();}e.updateHitBounds();
    }
    boolean hasFloorAt(EnemyEntity e,float x){for(Rectangle f:floors)if(x>=f.x&&x<=f.x+f.width&&Math.abs(e.bounds().y-(f.y+f.height))<7)return true;return false;}
    void start(int n){
        ...
        for(MapObject o:tiledMap.getLayers().get("interactiveObjects").getObjects())if(o instanceof RectangleMapObject){Rectangle r=((RectangleMapObject)o).getRectangle();String name=o.getName();if("diamond".equals(name)||"coin".equals(name))gems.add(new Collectible(r.x,r.y,r.width,r.height));else if("bonusBlock".equals(name))boxes.add(new Collectible(r.x,r.y,r.width,r.height));else if("ladder".equals(name))ladders.add(new Rectangle(r.x,r.y,r.width,r.height));else if("finishLine".equals(name))finish.set(r.x,r.y,r.width,r.height);}
        for(MapObject o:tiledMap.getLayers().get("actors").getObjects())if(o instanceof RectangleMapObject){Rectangle r=((RectangleMapObject)o).getRectangle();if("player".equals(o.getName())){px=r.x;py=r.y;player.setSize(34,38);}else if("enemyStar".equals(o.getName())){Object value=o.getProperties().get("walkLength");int tiles=value instanceof Number?((Number)value).intValue():value==null?0:Integer.parseInt(value.toString());foes.add(new EnemyEntity(r.x,r.y,r.x,r.x+tiles*32));}}
        vx=vy=0;session.mode(GameMode.PLAYING);
    }
    void add(float x,float y,float w,float h){floors.add(new Rectangle(x,y,w,h));}void startDeath(String text){playerDead=true;deathTime=0;endText=text;vx=vy=0;}void finish(boolean won,String text){endText=text;if(won){unlocked=Math.max(unlocked,Math.min(2,level+1));saveService.unlockedLevel(unlocked);}session.mode(GameMode.ENDED);}
    ...
    void drawMapWorld(){
        ...
        for(Collectible i:gems)if(!i.isCollected())batch.draw(diamondFrames[(int)(time*8)%4][0],i.bounds().x,i.bounds().y,i.bounds().width,i.bounds().height);
        for(Collectible i:boxes)if(!i.isCollected()){int frame=i.breakTime()<0?0:Math.min(7,4+(int)(i.breakTime()/.12f));batch.draw(crateFrames[0][frame],i.bounds().x,i.bounds().y,i.bounds().width,i.bounds().height);}
        for(EnemyEntity e:foes)if(!e.isDefeated())batch.draw(enemyFrames[e.patrolEnd()>e.patrolStart()?1:0][(int)(time*9)%(e.patrolEnd()>e.patrolStart()?6:8)],e.bounds().x,e.bounds().y,e.bounds().width,e.bounds().height);
    ...
    void pauseMenu(){shade();panel(410,250,460,230);center("GAME ON PAUSE",405,1.35f,Color.WHITE);button(470,300,190,48,"RESUME",new Color(.20f,.58f,.42f,1));button(680,300,130,48,"HOME",new Color(.78f,.38f,.25f,1));if(Gdx.input.justTouched()){if(touch(470,300,190,48))session.mode(GameMode.PLAYING);else if(touch(680,300,130,48))session.mode(GameMode.MENU);}}
    void ended(){shade();panel(380,230,520,280);center(endText.split("\\n")[0],420,1.25f,Color.WHITE);if(endText.contains("\n"))center(endText.split("\\n")[1],390,1.1f,Color.WHITE);center("Score: "+score,340,1f,Color.WHITE);button(535,260,210,48,"HOME",new Color(.20f,.58f,.42f,1));if(Gdx.input.justTouched()&&touch(535,260,210,48))session.mode(GameMode.MENU);}
