...
public class MyGdxGame extends ApplicationAdapter {
    ...
    Music music; Sound destroySound, attackSound; boolean musicEnabled, soundEnabled, russian;
    ...
    @Override public void create(){
        ...
        prefs=Gdx.app.getPreferences("pirate-treasure");unlocked=prefs.getInteger("unlocked",0);musicEnabled=prefs.getBoolean("music",true);soundEnabled=prefs.getBoolean("sound",true);russian=prefs.getBoolean("russian",false);music=Gdx.audio.newMusic(Gdx.files.internal("assets/sounds/background_music.mp3"));destroySound=Gdx.audio.newSound(Gdx.files.internal("assets/sounds/destroy.mp3"));attackSound=Gdx.audio.newSound(Gdx.files.internal("assets/sounds/shoot.mp3"));music.setLooping(true);if(musicEnabled)music.play();
    }
    ...
    void settings(){center(t("SETTINGS","НАСТРОЙКИ"),620,2f,new Color(.13f,.22f,.48f,1));panel(425,185,430,320);center(t("Progress and audio","ПРОГРЕСС И ЗВУК"),465,1f,Color.WHITE);button(480,405,320,40,t("RESET LEVELS","СБРОСИТЬ УРОВНИ"),new Color(.78f,.38f,.25f,1));button(480,355,320,40,t("MUSIC: ","МУЗЫКА: ")+(musicEnabled?"ON":"OFF"),new Color(.20f,.58f,.42f,1));button(480,305,320,40,t("SFX: ","ЗВУКИ: ")+(soundEnabled?"ON":"OFF"),new Color(.20f,.58f,.42f,1));button(480,255,320,40,t("LANGUAGE: ","ЯЗЫК: ")+(russian?"RU":"EN"),new Color(.20f,.58f,.42f,1));button(480,205,320,40,t("HOME","В МЕНЮ"),new Color(.78f,.38f,.25f,1));if(Gdx.input.justTouched()){if(touch(480,405,320,40)){unlocked=0;prefs.putInteger("unlocked",0).flush();}else if(touch(480,355,320,40)){musicEnabled=!musicEnabled;prefs.putBoolean("music",musicEnabled).flush();if(musicEnabled)music.play();else music.pause();}else if(touch(480,305,320,40)){soundEnabled=!soundEnabled;prefs.putBoolean("sound",soundEnabled).flush();}else if(touch(480,255,320,40)){russian=!russian;prefs.putBoolean("russian",russian).flush();}else if(touch(480,205,320,40))state=State.MENU;}}
    void drawMapWorld(){
