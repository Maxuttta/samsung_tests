...
public class MyGdxGame extends ApplicationAdapter {
    ...
    private GameMode state=GameMode.MENU;
    ...
    @Override public void render(){float dt=Math.min(Gdx.graphics.getDeltaTime(),.033f);time+=dt;if(state==GameMode.PLAYING)update(dt);Gdx.gl.glClearColor(.10f,.22f,.34f,1);Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);viewport.apply();if(state==GameMode.MENU||state==GameMode.SETTINGS){camera.zoom=1;camera.position.set(W/2,H/2,0);camera.update();batch.setProjectionMatrix(camera.combined);batch.begin();background();if(state==GameMode.MENU)menu();else settings();batch.end();return;}drawMapWorld();camera.zoom=1;camera.position.set(W/2,H/2,0);camera.update();batch.setProjectionMatrix(camera.combined);batch.begin();hud();if(state==GameMode.PAUSED)pauseMenu();if(state==GameMode.ENDED)ended();batch.end();}
    void update(float dt){
        ...
        if(player.overlaps(finish)){finish(true,"Our congratulations!");return;}if(py<-80){startDeath("Don't fall into the pit!");return;}if(Gdx.input.isKeyJustPressed(Input.Keys.ESCAPE)||Gdx.input.isKeyJustPressed(Input.Keys.P))state=GameMode.PAUSED;
    }
    ...
    void start(int n){
        ...
        vx=vy=0;state=GameMode.PLAYING;
    }
    void add(float x,float y,float w,float h){floors.add(new Rectangle(x,y,w,h));}void startDeath(String text){playerDead=true;deathTime=0;endText=text;vx=vy=0;}void finish(boolean won,String text){endText=text;if(won){unlocked=Math.max(unlocked,Math.min(2,level+1));saveService.unlockedLevel(unlocked);}state=GameMode.ENDED;}
